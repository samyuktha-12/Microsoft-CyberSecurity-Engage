...
/* The sys_call_table is const but we can point our own variable 
at its memory location to get around that.*/
...
{
    /* execute the original write call, and hold on to its return value now 
    we can add whatever we want to the buffer before exiting the function.*/
    long ret;
...
/* Too find the syscall table in memory
...
static unsigned long **aquire_sys_call_table(void)
{
	...
	while (offset < ULLONG_MAX) {
		...
		sct = (unsigned long **)offset;
        	...
        	/* We're scanning for a bit pattern that matches sct[__NR_close] so we just increase 'offset' until we find it.*/
